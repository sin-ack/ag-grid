...
export const parseCssDimension = (css: string): DimensionValue | null => {
  ...
  if (calcMatch) {
    ...
    const preparedExpression = expression.replace(/[a-z%]/gi, '');
    try {
      result = eval(preparedExpression) as unknown;
    } catch (e) {
      logErrorMessage(
        `Error evaluating expression "${expression}" as eval("${preparedExpression}")`,
        e,
      );
      return null;
    }
