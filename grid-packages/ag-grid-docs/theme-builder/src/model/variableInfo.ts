...
export type DimensionVariableInfo = {
  type: 'dimension';
  min: number;
  max: number;
  step: number;
...
export type ColorVariableInfo = {
  type: 'color';
};

export type BorderVariableInfo = {
  type: 'border';
  style?: boolean;
  width?: boolean;
  color?: boolean;
...
export type BorderStyleVariableInfo = {
  type: 'borderStyle';
};
...
const variables: Record<string, VariableInfo> = {
  ...
  '--ag-menu-min-width': { type: 'dimension', min: 0, max: 100, step: 1 },
  ...
  '--ag-side-bar-panel-width': { type: 'dimension', min: 0, max: 100, step: 1 },
